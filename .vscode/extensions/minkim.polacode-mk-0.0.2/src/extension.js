...
const writeSerializedBlobToFile = (serializeBlob, uri) => {
  const bytes = new Uint8Array(serializeBlob.split(','))
  vscode.workspace.fs.writeFile(uri, Buffer.from(bytes))
}
...
function activate(context) {
  ...
  let lastUsedImageUri = vscode.Uri.file(path.resolve(homedir(), `screenshot_${Date.now()}.png`))
  let panel
      ...
      switch (type) {
        case 'shoot':
          vscode.window
            ...
            .then(uri => {
              if (uri) {
                writeSerializedBlobToFile(data.serializedBlob, uri)
